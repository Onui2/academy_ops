...
export function requireAuthenticatedActor(request: Request): AuthenticatedActor {
  ...
  const appRole: AppRole = resolveAppRoleFromProfile(session.profile, session.portalRole);
  const legacyRole = resolveLegacyRoleFromProfile(session.profile, session.portalRole);
  return {
